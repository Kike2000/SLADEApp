...
namespace CongresoSlade.Infrastructure.Persistences.Repositories
{
    ...
    {
        public AreaRepository(CongresoServerDbContext context) : base(context)
        {
        }
        public async Task<BaseEntityResponse<Area>> ListAreas(BaseFilterRequest filters)
        {
            var response = new BaseEntityResponse<Area>();
